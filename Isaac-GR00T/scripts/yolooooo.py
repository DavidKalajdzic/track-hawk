...
DATA_DIR = "camera_data_test" + str(time.time())
...
try:
    while step <= 18120:
        ...
        log.append({
            ...
            "drone_orientation": np.array(airsim.to_eularian_angles(state.kinematics_estimated.orientation),
                                          dtype=np.float32),
            ...
            "target_position": np.array([pose.position.x_val, pose.position.y_val, pose.position.z_val],
                                        dtype=np.float32),
